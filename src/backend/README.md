...
**项目依赖于hoj-redis，voj-nacos，voj-mysql等镜像成功启动，以及根据前面三个镜像的配置修改环境参数才可正常启动**
...
services:
  voj-backend:
#    image: registry.cn-shanghai.aliyuncs.com/simplefanc/hoj_backend
	image: voj-backend
    ...
    environment:
      ...
      - MYSQL_PORT=3306 # voj-mysql端口号
      - MYSQL_DATABASE_NAME=voj # 改动需要修改hoj-mysql镜像,默认为hoj
      - MYSQL_USERNAME=root 
      - MYSQL_ROOT_PASSWORD=hoj123456 # voj-mysql的root账号密码
      ...
      - REDIS_PASSWORD=hoj123456 #voj-redis的密码
  ...
  voj-mysql:
    image: registry.cn-shanghai.aliyuncs.com/simplefanc/hoj_database
